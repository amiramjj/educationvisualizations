...
st.title("Comparison of Education Levels in Governorates")
...
st.title("Interactive District Education Data Visualization")

# Interactive Dropdown Menu to select education level
education_level = st.selectbox(
    "Select Education Level for Coloring Points:",
...
# Streamlit app layout
st.title("Scatter/Bubble Plot: Correlation between Illiteracy and School Dropout Rates")
...
# Streamlit app layout
st.title("Education Level Proportions Across Districts")
...
selected_district = st.selectbox("Select a District", options=district_data['refArea'].unique())

# Filter the data based on the selected district
highlight_data = district_data[district_data['refArea'] == selected_district]

# Pie chart for school dropout rates
fig_dropout = go.Figure(data=[go.Pie(
    labels=district_data['refArea'],
    values=district_data['PercentageofSchooldropout'],
    title="School Dropout Rates Across Districts",
    hole=0.3,  # Creates a donut chart effect
    sort=False,
    marker=dict(
        colors=['lightgrey' if x != selected_district else 'red' for x in district_data['refArea']]
    )
)])
...
fig_illeterate = go.Figure(data=[go.Pie(
    labels=district_data['refArea'],
    values=district_data['PercentageofEducationlevelofresidents-illeterate'],
